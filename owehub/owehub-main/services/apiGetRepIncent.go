...
func HandleGetRepIncentDataRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal get RepStatus data Request body", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to get RepStatus data from DB", http.StatusBadRequest, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "RepStatus Data", http.StatusOK, RepIncentList, RecordCount)
}
