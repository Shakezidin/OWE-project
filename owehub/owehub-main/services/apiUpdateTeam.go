 ...
 ******************************************************************************/
 func HandleUpdateTeamNameRequest(resp http.ResponseWriter, req *http.Request) {
	 ...
	 if err != nil {
		 ...
		 FormAndSendHttpResp(resp, "Failed to unmarshal update team name request", http.StatusBadRequest, nil)
		 return
	 }
	 ...
	 if updateData.TeamID <= 0 || updateData.TeamName == "" {
		 ...
		 log.FuncErrorTrace(0, "%v", err)
		 FormAndSendHttpResp(resp, "Invalid input fields in API request", http.StatusBadRequest, nil)
		 return
	 }
	 ...
	 if err != nil {
		 ...
		 FormAndSendHttpResp(resp, "Failed to update team name", http.StatusInternalServerError, nil)
		 return
	 }
 
	 FormAndSendHttpResp(resp, "Team name updated successfully", http.StatusOK, nil)
 }
