...
func HandleUpdateDLROTHArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal update DLROTH archive request", http.StatusBadRequest, nil)
		return
	}
	...
	if len(updateDLROTHArcReq.RecordId) <= 0 {
		...
		log.FuncErrorTrace(0, "%v", err)
		FormAndSendHttpResp(resp, "Record Id is empty, update DLROTH archive failed", http.StatusBadRequest, nil)
		return
	}
