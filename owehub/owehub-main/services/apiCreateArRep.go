...
func HandleCreateArRepRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Create Ap-Rep", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "ap-Rep Created Successfully", http.StatusOK, nil)
}
