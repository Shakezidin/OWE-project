...
/******************************************************************************
 * FUNCTION:		HandleRepCreditArchiveRequest
 * DESCRIPTION:     handler for  RepCredit Archive request
 * INPUT:			resp, req
 * RETURNS:    		void
 ******************************************************************************/

func HandleRepCreditArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	var (
		err             error
		RepCreditArcReq models.ArchiveRepCredit
		queryParameters []interface{}
		result          []interface{}
	)

	log.EnterFn(0, "HandleRepCreditArchiveRequest")
	defer func() { log.ExitFn(0, "HandleRepCreditArchiveRequest", err) }()
	...
	err = json.Unmarshal(reqBody, &RepCreditArcReq)
	if err != nil {
	...
	if len(RepCreditArcReq.RecordId) <= 0 {
	...
	for _, id := range RepCreditArcReq.RecordId {
	...
	queryParameters = append(queryParameters, RepCreditArcReq.IsArchived)

	// Call the database function
	result, err = db.CallDBFunction(db.OweHubDbIndex, db.UpdateRepCreditArchiveFunction, queryParameters)
