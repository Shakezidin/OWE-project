...
func HandleUpdateRepCreditRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if UpdateRepCreditReq.PerKwAmt <= float64(0) {
		...
		log.FuncErrorTrace(0, "%v", err)
		FormAndSendHttpResp(resp, "Invalid Values Not Allowed", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Update RepCredit", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "RepCredit Updated Successfully", http.StatusOK, nil)
}
