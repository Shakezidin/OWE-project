...
func HandleApAdvArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	var (
		err             error
		ApAdvArcReq     models.ArchiveApAdv
		queryParameters []interface{}
		result          []interface{}
	)

	log.EnterFn(0, "HandleApAdvArchiveRequest")
	defer func() { log.ExitFn(0, "HandleApAdvArchiveRequest", err) }()
	...
	err = json.Unmarshal(reqBody, &ApAdvArcReq)
	if err != nil {
	...
	for _, id := range ApAdvArcReq.RecordId {
	...
	queryParameters = append(queryParameters, ApAdvArcReq.IsArchived)

	// Call the database function
	result, err = db.CallDBFunction(db.OweHubDbIndex, db.UpdateApAdvArchiveFunction, queryParameters)
