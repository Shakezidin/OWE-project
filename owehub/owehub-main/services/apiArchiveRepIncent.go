...
/******************************************************************************
 * FUNCTION:		HandleRepIncentArchiveRequest
 * DESCRIPTION:     handler for  RepIncent Archive request
 * INPUT:			resp, req
 * RETURNS:    		void
 ******************************************************************************/

func HandleRepIncentArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	var (
		err             error
		RepIncentArcReq models.ArchiveRepIncent
		queryParameters []interface{}
		result          []interface{}
	)

	log.EnterFn(0, "HandleRepIncentArchiveRequest")
	defer func() { log.ExitFn(0, "HandleRepIncentArchiveRequest", err) }()
	...
	err = json.Unmarshal(reqBody, &RepIncentArcReq)
	if err != nil {
	...
	if len(RepIncentArcReq.RecordId) <= 0 {
	...
	for _, id := range RepIncentArcReq.RecordId {
	...
	queryParameters = append(queryParameters, RepIncentArcReq.IsArchived)

	// Call the database function
	result, err = db.CallDBFunction(db.OweHubDbIndex, db.UpdateRepIncentArchiveFunction, queryParameters)
