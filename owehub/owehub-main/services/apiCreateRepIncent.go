...
func HandleCreateRepIncentRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Create RepIncent", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "RepIncent Created Successfully", http.StatusOK, nil)
}
