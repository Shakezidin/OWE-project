...
func HandleCreateRepStatusRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Create RepStatus", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "RepStatus Created Successfully", http.StatusOK, nil)
}
