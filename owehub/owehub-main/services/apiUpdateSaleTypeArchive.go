...
func HandleUpdateSaleTypeArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal update sale type Archive request", http.StatusBadRequest, nil)
		return
	}
	...
	if len(updateSaleTypeArcReq.RecordId) <= 0 {
		...
		log.FuncErrorTrace(0, "%v", err)
		FormAndSendHttpResp(resp, "Record Id is empty, Update sale type Archive failed", http.StatusBadRequest, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "sale type Archive Updated Successfully", http.StatusOK, nil)
}
