...
func HandleUpdateRepIncentRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Update RepIncent", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "RepIncent Updated Successfully", http.StatusOK, nil)
}
