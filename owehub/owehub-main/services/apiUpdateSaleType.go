...
func HandleUpdateSaleTypeRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal update sale type request", http.StatusBadRequest, nil)
		return
	}
	...
	result, err = db.CallDBFunction(db.UpdateSaleTypeFunction, queryParameters)
	if err != nil || len(result) <= 0 {
		log.FuncErrorTrace(0, "Failed to Update sale type in DB with err: %v", err)
		FormAndSendHttpResp(resp, "Failed to Update sale type", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "sale type Updated Successfully", http.StatusOK, nil)
}
