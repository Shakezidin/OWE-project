...
func HandleGetVAdderDataRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		appserver.FormAndSendHttpResp(resp, "Failed to unmarshal get v adders data Request body", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil {
		...
		appserver.FormAndSendHttpResp(resp, "Failed to get v adders data from DB", http.StatusBadRequest, nil)
		return
	}
	...
	appserver.FormAndSendHttpResp(resp, "V Adders Data", http.StatusOK, vaddersList, RecordCount)
}
