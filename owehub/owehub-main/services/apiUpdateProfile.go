...
func HandleUpdateProfileRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal update Profile request", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Update Profile", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "Profile Updated Successfully", http.StatusOK, nil)
}
