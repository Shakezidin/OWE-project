...
func HandleUpdateTimelineSlaArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal update TimelineSla Archive request", http.StatusBadRequest, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "TimelineSla Archive Updated Successfully", http.StatusOK, nil)
}
