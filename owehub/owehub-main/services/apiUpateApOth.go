...
func HandleUpdateApOthRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Update ApOth", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "ApOth Updated Successfully", http.StatusOK, nil)
}
