...
func HandleUpdateReconcileRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal update ReconcileData request", http.StatusBadRequest, nil)
		return
	}
	...
	queryParameters = append(queryParameters, updateReconcileReq.StartDate)
	queryParameters = append(queryParameters, updateReconcileReq.EndDate)
	...
	result, err = db.CallDBFunction(db.UpdateReconcileFunction, queryParameters)
	if err != nil || len(result) <= 0 {
		log.FuncErrorTrace(0, "Failed to Add Reconcile data in DB with err: %v", err)
		FormAndSendHttpResp(resp, "Failed to update Reconcile data", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "ReconcileData updated Successfully", http.StatusOK, nil)
}
