...
func HandleUpdateVDealerDataRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal Update v dealer request", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Update V dealer", http.StatusInternalServerError, nil)
		return
	}
