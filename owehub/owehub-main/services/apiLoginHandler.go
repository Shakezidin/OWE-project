...
func HandleLoginRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if !nonDealerRoles[roleName] {
		query := fmt.Sprintf("SELECT sp.sales_partner_name as dealer_name FROM user_details ud JOIN sales_partner_dbhub_schema sp ON sp.item_id = ud.partner_id WHERE ud.email_id = '%v'", emailId)
