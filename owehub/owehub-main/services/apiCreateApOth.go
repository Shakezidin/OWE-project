...
func HandleCreateApOthRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Create ApOth", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "ApOth Created Successfully", http.StatusOK, nil)
}
