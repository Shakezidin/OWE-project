...
func HandleUpdateApptSettersArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		appserver.FormAndSendHttpResp(resp, "Failed to unmarshal update appt setters archive request", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil || len(result) <= 0 {
		...
		appserver.FormAndSendHttpResp(resp, "Failed to Update appt setters archive", http.StatusInternalServerError, nil)
		return
	}
	data := result[0].(map[string]interface{})

	log.DBTransDebugTrace(0, "appt setters archive updated with Id: %+v", data)
	appserver.FormAndSendHttpResp(resp, "Appt Setters Archive Updated Successfully", http.StatusOK, nil)
}
