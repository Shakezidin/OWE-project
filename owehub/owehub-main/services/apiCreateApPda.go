...
func HandleCreateApPdaRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil || len(result) <= 0 {
		...
		FormAndSendHttpResp(resp, "Failed to Create ApPda", http.StatusInternalServerError, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "ApPda Created Successfully", http.StatusOK, nil)
}
