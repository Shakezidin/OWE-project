...
func GetperformerProfileDataRequest(resp http.ResponseWriter, req *http.Request) {
	...
	query = fmt.Sprintf("SELECT COUNT(system_size) AS total_sales, COUNT(ntp_date) AS total_ntp, COUNT(project_status) as install FROM consolidated_data_view WHERE dealer = '%d' AND primary_sales_rep = '%d' AND ntp_date IS NOT NULL AND project_status = 'INSTALL' AND ")
