...
func HandleGetApAdvDataRequest(resp http.ResponseWriter, req *http.Request) {
	...
	query = `SELECT 
	 ap.unique_id, ap.payee, ap.customer, ap.amount_ovrd, ap.date, ap.customer, ap.approved_by, ap.notes, vd.dealer_code as dealer
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to get ar schedule data from DB", http.StatusBadRequest, nil)
		return
	}
	...
	FormAndSendHttpResp(resp, "Ar Schedule Data", http.StatusOK, ApAdvList, RecordCount)
}
...
func PrepareApAdvFilters(tableName string, dataFilter models.DataRequestBody, forDataCount bool) (filters string, whereEleList []interface{}) {
	...
	if forDataCount == true {
		filtersBuilder.WriteString(" GROUP BY ap.unique_id, ap.payee, ap.customer, ap.amount_ovrd, ap.date, ap.customer, ap.approved_by, ap.notes, vd.dealer_code as dealer")
	} else {
