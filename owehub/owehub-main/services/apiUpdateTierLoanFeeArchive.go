...
func HandleUpdateTierLoanFeeArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		appserver.FormAndSendHttpResp(resp, "Failed to unmarshal update tier loan fee Archive request", http.StatusBadRequest, nil)
		return
	}
	...
	if len(updateTierLoanFeeArcReq.RecordId) <= 0 {
		...
		log.FuncErrorTrace(0, "%v", err)
		appserver.FormAndSendHttpResp(resp, "Record Id is empty, Update tier loan fee Archive failed", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil || len(result) <= 0 {
		...
		appserver.FormAndSendHttpResp(resp, "Failed to Update tier loan fee Archive", http.StatusInternalServerError, nil)
		return
	}
	data := result[0].(map[string]interface{})

	log.DBTransDebugTrace(0, "tier loan fee Archive updated with Id: %+v", data)
	appserver.FormAndSendHttpResp(resp, "tier loan fee Archive Updated Successfully", http.StatusOK, nil)
}
