...
/******************************************************************************
 * FUNCTION:		HandleApOthArchiveRequest
 * DESCRIPTION:     handler for  ApOth Archive request
 * INPUT:			resp, req
 * RETURNS:    		void
 ******************************************************************************/

func HandleApOthArchiveRequest(resp http.ResponseWriter, req *http.Request) {
	var (
		err             error
		ApOthArcReq     models.ArchiveApOth
		queryParameters []interface{}
		result          []interface{}
	)

	log.EnterFn(0, "HandleApOthArchiveRequest")
	defer func() { log.ExitFn(0, "HandleApOthArchiveRequest", err) }()
	...
	err = json.Unmarshal(reqBody, &ApOthArcReq)
	if err != nil {
	...
	for _, id := range ApOthArcReq.RecordId {
	...
	queryParameters = append(queryParameters, ApOthArcReq.IsArchived)

	// Call the database function
	result, err = db.CallDBFunction(db.OweHubDbIndex, db.UpdateApOthArchiveFunction, queryParameters)
