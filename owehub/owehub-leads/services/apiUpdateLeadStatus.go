...
func HandleUpdateLeadStatusRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		...
		FormAndSendHttpResp(resp, "Failed to update lead status", http.StatusInternalServerError, nil)
		return
	}
	...
	if !ok {
		...
		FormAndSendHttpResp(resp, "Failed to get the user_id from database", http.StatusInternalServerError, nil)
		return
	}
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to get lead details from database", http.StatusInternalServerError, nil)
		return
	}
	if len(data) <= 0 {
	...
	if dataReq.StatusId == 1 {
		...
		if leadStatus != 0 && leadStatus != 1 && leadStatus != 3 && leadStatus != 4 {
			...
			FormAndSendHttpResp(resp, "Invalid update lead action", http.StatusBadRequest, nil)
			return
		}
		...
		if err != nil {
			...
			FormAndSendHttpResp(resp, "Failed to send the email to the lead", http.StatusInternalServerError, nil, 0)
			return
		}
		...
		if err != nil {
			...
			FormAndSendHttpResp(resp, "Failed to update the lead details in db", http.StatusInternalServerError, nil)
			return
		}
		FormAndSendHttpResp(resp, "Appointment Sent", http.StatusOK, nil, 0)
		return
	}
	...
	if dataReq.StatusId == 5 {
		...
		if !ok {
			...
			FormAndSendHttpResp(resp, "Failed to get the appointment_date from database", http.StatusInternalServerError, nil)
			return
		}
		...
		FormAndSendHttpResp(resp, "Status Updated", http.StatusOK, nil, 0)
		return
	}
	...
	if dataReq.StatusId == 6 {
		...
		if dataReq.Reason == "" {
			...
			FormAndSendHttpResp(resp, "Reason is required", http.StatusBadRequest, nil)
			return
		}
