...
func HandleGetLeadsCountByStatusRequest(resp http.ResponseWriter, req *http.Request) {
	...
	`
	data, err = db.ReteriveFromDB(db.OweHubDbIndex, query, []interface{}{authenticatedEmail, startDate, endDate})
	...
	for _, item := range data {
		...
		if statusId == 4 {
			query = `
				...
				WHERE (li.status_id = 5 AND li.proposal_created_date IS NULL)
				OR (li.status_id NOT IN (5, 6) AND li.appointment_date < CURRENT_TIMESTAMP);
			`
