...
export const fetchAdderResponsibility = createAsyncThunk(
  ...
  export const updateAdderResponsibility = createAsyncThunk("update/adder-responsibility",async(params:any,{rejectWithValue,dispatch})=>{
    try {
        const data = await postCaller("update_adder_responsibility",params)
        await dispatch(fetchAdderResponsibility({page_number:1,page_size:10}))
        return data.data
    } catch (error) {
        return rejectWithValue((error as Error).message)
    }
