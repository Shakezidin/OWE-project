...
import { fetchAdderCredit,createAdderCredit,updateAdderCredit} from "../../../apiActions/adderCreditAction";
...
const adderCredit = createSlice({
    ...
    extraReducers: builder => {
        builder.addCase(fetchAdderCredit.pending, (state) => {
            state.isLoading = true
        })
            .addCase(fetchAdderCredit.fulfilled, (state, action: PayloadAction<any | null>) => {
                state.isLoading = false
                state.data = action.payload ? action.payload.data.adder_credit_list  :[]
            })
