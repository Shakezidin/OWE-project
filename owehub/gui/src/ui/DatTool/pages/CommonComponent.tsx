...
const CommonComponent: React.FC<commonComponentProps> = ({
  ...
  const [address, setAddress] = useState({
    value: generalData?.project_address,
    changed: false,
  });
  const [phoneNumber, setPhoneNumber] = useState({
    value: generalData?.phone_number,
    changed: false,
  });
  const [emailId, setEmailId] = useState({
    value: generalData?.email_id,
    changed: false,
  });
  ...
  const systemMetrics = [
    ...
    {
      value: generalData?.battery_capacity || '---',
  ...
  const integrations = [
    { icon: ICONS.DatAurora, name: 'Aurora', link: generalData?.aurora_link },
    { icon: ICONS.DatTape, name: 'Tape', link: generalData?.tape_link },
    {
      icon: ICONS.DatCapture,
      name: 'Site Capture',
      link: generalData?.site_capture_url,
    },
  ];
          ...
          <div className={styles.genOneLeft}>
                ...
                {[
                  {
                    ...
                    onChange: (e: React.ChangeEvent<HTMLInputElement>) =>
                      valueChangeHandler(e, 'text'),
                  },
                  {
                    type: 'tel',
                    value: phoneNumber,
                    onChange: (e: React.ChangeEvent<HTMLInputElement>) =>
                      valueChangeHandler(e, 'tel'),
                  },
                  {
                    type: 'email',
                    value: emailId,
                    onChange: (e: React.ChangeEvent<HTMLInputElement>) =>
                      valueChangeHandler(e, 'email'),
                  },
                ].map((input, index) => (
                  <input
                    key={index}
            ...
            {isMobile && (
              <div
                ...
                style={{ bottom: openTab ? '-4%' : '-12%' }}
              >
                {openTab ? (
                  <FaChevronUp onClick={() => setOpenTab((prev) => !prev)} />
                ) : (
                  <FaChevronDown onClick={() => setOpenTab((prev) => !prev)} />
                )}
              </div>
            )}
          </div>
