...
const [values, setValues] = useState<{ [key: number]: number[] }>({
  ...
  const handleIncrement = (index: number) => {
    ...
    setPrice((prevPrice) => {
      ...
      newPrice[currentSectionIndex] = (newPrice[currentSectionIndex] || 0) + itemPrice;
  ...
  const handleDecrement = (index: number) => {
    setValues((prevValues) => {
      const newValues = { ...prevValues };
      newValues[currentSectionIndex][index] = Math.max((newValues[currentSectionIndex][index] || 0) - 1, 0); 
      return newValues;
    });
  ...
  useEffect(() => {
    ...
    const totalPrice = Object.values(price).reduce((acc, priceValue) => acc + priceValue, 0);
    setTotal(totalPrice);
  }, [price]);
    ...
    <div style={{ padding: "0 1.2rem", position: "relative", height: "calc(100vh - 216px)", overflowY: "scroll" }}>
      ...
      <div className={styles.adderssPageTopPart}>
        ...
        <div className={styles.adderssPageTopPart_rightText}>
          <p className={styles.adderssPageTopPart_rightText_amount}>$ {total}</p>
        ...
        <div className={styles.adderssPageMainPart_left}>
          {leftPartObj.map((bar, index) => (
            <div
              ...
              key={index}
              style={{ backgroundColor: currentSectionIndex === index ? "#377CF6" : "" }}
              onClick={() => handleSectionIndex(index)}
            >
              <p className={styles.adderssPageMainPart_leftBarContainer_heading} style={{ color: currentSectionIndex === index ? "#FFFFFF" : "" }}>
                {bar.text}
              </p>
              <div className={styles.adderssPageMainPart_leftBarContainer_right}>
                ...
                <p className={styles.adderssPageMainPart_leftBarContainer_price} style={{ color: currentSectionIndex === index ? "#FFFFFF" : "" }}>$ {price[index]}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className={styles.adderssPageMainPart_rightBarContainer} key={index}>
              ...
              <div className={styles.adderssPageMainPart_rightContainer}>
                <div className={styles.adderssPageMainPart_expressionContainer}>
                  <FiMinus className={styles.adderssPageMainPart_decrement} onClick={
                    () => handleDecrement(index)
                    } />
                  <p className={styles.adderssPageMainPart_text}>{values[currentSectionIndex]?.[index] || 0}</p>
                  <FiPlus className={styles.adderssPageMainPart_increment} onClick={() => handleIncrement(index)} />
                </div>
