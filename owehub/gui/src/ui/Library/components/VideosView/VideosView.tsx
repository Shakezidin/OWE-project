...
function VideosView({ videoData, onClick }: VideosViewProps) {
  ...
  const [isHoveredDownload, setIsHoveredDownload] = useState<number | null>(
    null
  );
    ...
    <div className={styles.folderMain_wrapper}>
      {videos.map((Video: VideoData, index: number) => (
        <div
          ...
          key={index}
          onClick={() =>
            onClick(Video['@microsoft.graph.downloadUrl'], Video.name)
          }
        >
          <div
            className={
              videos.length > 6
                ? styles.videoImage_wrapper
                : styles.videoImage_wrapperr
            }
          >
          ...
          <div className={styles.folderContent_wrapper}>
            ...
            >
              <div className={styles.videosview_name}>
                {Video.name.substring(0, 15)}{' '}
                {parseInt(Video.name) >= 16 ? '...' : ''}
              </div>
              ...
              <div className={styles.vidIcons}>
                <RiDeleteBinLine
                  ...
                  style={{
                    height: '14px',
                    width: '14px',
                    color: isHovered === index ? '#377CF6' : '#101828',
                    cursor: 'pointer',
                  }}
                ...
                <RxDownload
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    downloadFile(
                      Video['@microsoft.graph.downloadUrl'],
                      Video.name
                    );
                  }}
                  style={{
                    height: '14px',
                    width: '14px',
                    color: isHoveredDownload === index ? '#377CF6' : '#101828',
                    cursor: 'pointer',
                  }}
            ...
            <div className={styles.videoInfo_wrapper}>
              <div className={styles.vidTextWrapper}>
                ...
                <div className={styles.videosdate}>
                  {(() => {
                    const dateString = Video.lastModifiedDateTime.substring(
                      0,
                      10
                    ); // Get "YYYY-MM-DD"
