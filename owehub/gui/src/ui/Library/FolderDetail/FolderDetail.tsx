  ...
  return (
    <div className={`relative ${styles.libraryContainer}`}>
      <div className={` ${containerTyles.lg_hide} ${styles.libraryHeader}`}>
        <div
          className={`  ${styles.libSecHeader_right} ${containerTyles.mobile_gap}`}
          style={{ width: '100%' }}
        >
          <div
            className={` bg-white ${containerTyles.flex_auto} ${containerTyles.search_container} ${styles.searchWrapper}`}
          >
      ...
      {selected.size ? (
        <div className="flex items-center justify-between p2 bg-white">
          ...
          <div className={styles.delete_right}>
            <button
              disabled={isPending}
              onClick={() => setMultiDeletePopup(true)}
        ...
        <div className={styles.libSecHeader}>
          <div className={styles.folderHeader}>
            ...
            <p className={styles.recycle_p}>
              {' '}
              {parent?.[parent?.length - 1] || 'Unknown'}{' '}
            </p>
          </div>

          <div className={`  ${styles.libSecHeader_right}`}>
            <div
              className={` ${containerTyles.mobile_hide} ${styles.searchWrapper}`}
            >
              <IoMdSearch className={styles.search_icon} />
              {/* SEARCHINGGGG */}
              <input
        ...
        <div className={styles.libSectionWrapper}>
          {viewMode === 'list' && (
            <div className={styles.lib_Grid_Header}>
              ...
              <div
                className={`${styles.grid_item_upload_date} ${styles.sm_hide} ${styles.grid_item}`}
              >
                Uploaded Date
              </div>
            ...
            viewMode === 'list' ? (
              <div className="bg-white" style={{ minHeight: '63vh' }}>
                {paginatedData.map((file, index) => {
                  const fileType = getContentThumbnail(
                    file.folder ? 'folder' : file.file?.mimeType!
                  );
                  const isValidVideo = isVideo(file.file?.mimeType!);
                  return (
                    <div key={file.id} className={styles.libGridItem}>
                      {file.folder ? (
                        <div className="flex items-center">
                          {role_name === TYPE_OF_USER.ADMIN && (
                            <div className="mr1">
                          ...
                          >
                            ...
                            <div className={styles.name_div}>
                              ...
                              />
                              <div
                                className={`${containerTyles.lg_text_hide}  ${styles.grid_item_dates} `}
                                style={{
                                  fontSize: '12px',
                        ...
                        <div className="flex items-center">
                          ...
                          <div
                            ...
                            onClick={() => {
                              ...
                              if (
                                isImage(file?.file?.mimeType!) ||
                                isAudio(file.file?.mimeType!)
                              ) {
                                setFileInfo({
                                  name: file.name,
                      ...
                      <div
                        className={`${styles.grid_item} ${styles.grid_item_upload_date}  ${styles.sm_hide}  `}
                      >
                        {format(
                          new Date(file.lastModifiedDateTime),
                          'dd-MM-yyyy'
                      ...
                      <div
                        className={`${styles.grid_item} ${styles.grid_icon}`}
                      >
                        <RxDownload
                          ...
                          style={{
                            ...
                            width: '18px',
                            color:
                              isHovered === index && !file.folder
                                ? '#377CF6'
                                : file.folder
                                  ? 'rgba(102, 112, 133, 0.5)'
                                  : '#101828',
                            cursor: !file.folder ? 'pointer' : 'not-allowed',
                          }}
                          onClick={() =>
                            !file.folder &&
                            downloadFile(
                  ...
                  {paginatedData.map((file) => {
                    return !file?.folder ? (
                      <FileTileView
                        ...
                        onFilePreview={onPreview}
                        selected={selected}
                        onCheck={(id) => handleCheckboxChange(id)}
                        key={file.id}
                      />
                    ) : (
                      <div
                        ...
                        onClick={() =>
                          isTablet
                            ? navigate(`/library/${path['*']}/${file.name}`)
                            : undefined
                        }
                        onDoubleClick={() =>
                          isTablet
                      ...
                      >
                        <div
                          className={folderWrapperStyles.createdByWrapper}
                        ></div>
                        <div className={tileViewStyles.thumbnail_wrapper}>
                          <div className={folderWrapperStyles.charDiv}>
                            {file.name.charAt(0)}
                          </div>
                          <img src={ICONS.folderImage} alt="" />
                          <div className={folderWrapperStyles.checkboxWrapper}>
                            <p className={folderWrapperStyles.quantity}>
                              {file.childCount}
                            </p>
                            {role_name === TYPE_OF_USER.ADMIN && (
                              <div
                                className={` ${selected.has(file?.id!) || isTablet ? tileViewStyles.selected : ''}  ${tileViewStyles.checkbox_wrapper}`}
                              >
                                <CheckBox
                                  onChange={() => {
                        ...
                        <div className={'mt2'} style={{ width: '100%' }}>
                          <div className={styles.name_div}>
                            <div
                              data-tooltip-id={`file-name-${file.id}`}
                              data-tooltip-content={file.name}
                              className={folderWrapperStyles.folder_name}
                            >
                          ...
                          <div
                            className={folderWrapperStyles.folderInfo_wrapper}
                          >
                            <div
                              className={`  ${folderWrapperStyles.folderdate}`}
                            >
                              {format(
                                new Date(file.lastModifiedDateTime),
                                'dd-MM-yyyy'
          ...
          {!!paginatedData.length && (
            <div
              ...
              className="page-heading-container bg-white"
            >
              <p className="page-heading">
                Showing {startIndex} -{' '}
                {endIndex > files.length ? files.length : endIndex} of{' '}
                {files.length} item
              </p>
      ...
      {multiDeletePopup && (
        <DeleteFileModal
          ...
          setIsVisible={setMultiDeletePopup}
        />
      )}
      {isVideoModalOpen && (
        <VideoPlayer
