...
const LibraryHomepage = () => {
  ...
  const [filesView, setFilesView] = useState<'list' | 'tiles'>(
    (localStorage.getItem('fileTypeView') as 'list' | 'tiles') || 'tiles'
  );
  ...
  const [currentFolderContent, setCurrentFolderContent] = useState<
    FileOrFolder[]
  >([]);
  const { microsoftGraphAccessToken, role_name } = useAppSelector(
    (state) => state.auth
  );
  ...
  const [selectedCheckbox, setSelectedCheckbox] = useState<Set<string>>(
    new Set()
  );
  ...
  const fetchDataFromGraphAPI = async () => {
    setLoading(true);
    const url =
      'https://graph.microsoft.com/v1.0/sites/e52a24ce-add5-45f6-aec8-fb2535aaa68e/drive/root/children'; //endpoint
  ...
  const HandleSearch = (e: any) => {
    ...
    if (
      inputValue.length > 0 &&
      (inputValue.charAt(0) === ' ' ||
        !validCharacters.test(inputValue.charAt(0)))
    ) {
  ...
  const filteredData = fileData.filter((data) => {
    const matchesSearch =
      ...
      data.lastModifiedBy.user.displayName
        .toLowerCase()
    ...
    const matchesType =
      ...
      (selectedType === 'PDF Format' &&
        mimeType &&
        pdfMimes.includes(mimeType)) ||
      (selectedType === 'Images' &&
        mimeType &&
        imageMimes.includes(mimeType)) ||
      (selectedType === 'Videos' &&
        mimeType &&
        videoMimes.includes(mimeType)) ||
      (selectedType === 'Text' && mimeType && textMimes.includes(mimeType)) ||
      (selectedType === 'Powerpoint' &&
        mimeType &&
        powerpointMimes.includes(mimeType)) ||
  ...
  const paginatedFolderData = getPaginatedData(
    sortedFolder,
    currentFolderPage,
    itemsPerPage
  );
  ...
  const handleDeleteFiles = async () => {
    setIsPending(true);
    Promise.all(
      Array.from(selectedCheckbox).map((id) => DeleteHandler(id as string))
    )
      .then((res) => {
        toast.success(
          `Deleted ${res.length} ${res.length > 1 ? 'files' : 'file'}`
        );
        reset();
        setCheckedItems(0);
        fetchDataFromGraphAPI();
  ...
  const handleDelete = async () => {
    ...
    Promise.all(allIds.map((id) => DeleteHandler(id)))
      .then((res) => {
        ...
        setCheckedFolders([]);
        toast.success(
          `Deleted ${res.length} ${res.length > 1 ? 'folders' : 'folder'}`
        );
        fetchDataFromGraphAPI();
      })
      .catch((err) => {
        ...
        <>
          <div className={styles.delete_left}>
            ...
            <span className={styles.selectedCount}>
              {activeSection === 'files' ? selectedCheckbox.size : checkedItems}{' '}
              {activeSection === 'files' ? 'file' : 'folder'}
              {checkedItems > 1 || selectedCheckbox.size > 1 ? 's ' : ' '}
              selected
            </span>
          </div>
    ...
    return (
      <>
        <div className={styles.libSecHeader_left}>
          ...
          {selectedType !== 'All' && activeSection !== 'folders' ? (
        ...
        <div className={`  ${styles.libSecHeader_right}`}>
          ...
          {role_name === TYPE_OF_USER.ADMIN && (
            <NewFile
              ...
              handleSuccess={fetchDataFromGraphAPI}
  ...
  const renderContent = () => {
    if (activeSection === 'folders') {
      return filesView === 'list' ? (
        <FolderListView
          folders={paginatedFolderData.map((item) => ({
            name: item.name,
            size: item.size,
      ...
      <div className={styles.libSectionWrapper}>
              ...
              <div className="flex items-center">
                {role_name === TYPE_OF_USER.ADMIN && (
                  <div className="mr1">
                    <CheckBox
                      checked={
                        selectedCheckbox.size === paginatedData.length &&
                        !loading &&
                ...
                <span className={styles.libname_heading}>Name</span>
              </div>
            </div>
            <div className={` ${styles.sm_hide} ${styles.grid_item}`}>
              Uploaded Date
            </div>

            <div className={`${styles.grid_item} ${styles.grid_item_action}`}>
              Actions
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          filesView === 'list' ? (
            (selectedType === 'Videos'
              ? getPaginatedData(
                  sortedData.filter((item) => isVideo(item.file?.mimeType!)),
                  currentPage,
                  itemsPerPage
                )
              : paginatedData
                ...
                <div className={styles.libGridItem} key={data.id}>
                  <div className="flex items-center">
                    {role_name === TYPE_OF_USER.ADMIN && (
                      <div className="mr2">
                        <CheckBox
                          checked={selectedCheckbox.has(data.id)}
                          onChange={() => {
                            if (selectedCheckbox.has(data.id)) {
                              const newArr = new Set(
                                Array.from(selectedCheckbox).filter(
                                  (item) => item !== data.id
                                )
                              );
                    ...
                    <div
                      ...
                      className={`${styles.file_icon} ${styles.image_div}`}
                      onClick={() => {
                        if (isValidVideo) {
                    ...
                    >
                      ...
                      <div className={styles.name_div}>
                        <p
                          data-tooltip-id={`file-name-${data.id}`}
                        ...
                        <Tooltip
                          ...
                          id={`file-name-${data.id}`}
                          place="top"
                        />
                        <div className={styles.size_date_container}>
                          <div
                            className={` ${styles.sm_hide_upload_date} ${styles.grid_item_dates} `}
                            style={{ fontSize: '12px' }}
                          >
                            {format(
                  ...
                  <div
                    className={` ${styles.sm_hide} ${styles.grid_item_dates}`}
                  >
                    {format(new Date(data.lastModifiedDateTime), 'dd-MM-yyyy')}
                  </div>
                  <div
                    className={`${styles.grid_item_delete} ${styles.grid_icon} justify-center`}
                  >
                    <div>
                      <RxDownload
  ...
  return (
    <div className={styles.libraryContainer}>
      <div
        className={` items-center ${styles.desktop_hide}`}
        style={{ gap: 8 }}
      >
      ...
      >
        ...
        {(activeSection === 'files' ? !!sortedData.length : false) && (
          <div className="page-heading-container ">
            <p className="page-heading">
              Showing {startIndex} -{' '}
              {endIndex > sortedData.length ? sortedData.length : endIndex} of{' '}
        ...
        {(activeSection === 'folders' ? !!sortedFolder.length : false) && (
          <div className="page-heading-container ">
            <p className="page-heading">
              Showing {folderStartIndex} -{' '}
              {folderEndIndex > sortedFolder.length
                ? sortedFolder.length
                : folderEndIndex}{' '}
      ...
      {isVisible && (
        <DeleteFileModal
          ...
          onDelete={() =>
            activeSection === 'folders' ? handleDelete() : handleDeleteFiles()
          }
        />
      )}
    </div>
