...
func callApi(method string, apiPath string, reqBody interface{}, respBody interface{}) error {
	...
	apiUrl := AuroraCfg.AuroraApiBaseUrl +
		strings.ReplaceAll(apiPath, "{tenant_id}", AuroraCfg.AuroraTenantId)
	...
	// set headers
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", AuroraCfg.AuroraBearerToken))
