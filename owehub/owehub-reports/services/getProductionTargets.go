...
func HandleGetProductionTargets(resp http.ResponseWriter, req *http.Request) {
	...
	case "am":
		query = `
            ...
            WHERE p.target_percentage = $1 AND p.year = $2 AND p.month = $3 AND LOWER(p.state) = 'all'
            GROUP BY u.user_name, p.user_id
        `
		whereEleList = []interface{}{dataReq.TargetPercentage, dataReq.Year, dataReq.Month}
	...
	for _, item := range data {
		...
		if dataReq.GroupBy == "month" {
			...
			respItem.UserName, _ = item["user_name"].(string)
