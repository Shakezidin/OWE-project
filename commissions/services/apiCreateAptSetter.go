...
func HandleCreateAptSetterRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal create appointment setter request", http.StatusBadRequest, nil)
		return
	}
	...
	if createAptSetterReq.PayRate <= float32(0) {
		...
		log.FuncErrorTrace(0, "%v", err)
		FormAndSendHttpResp(resp, "Invalid Pay Rate Not Allowed", http.StatusBadRequest, nil)
		return
	}
