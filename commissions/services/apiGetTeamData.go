...
func HandleGetTeamDataRequest(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to unmarshal get teams data Request body ", http.StatusBadRequest, nil)
		return
	}
	...
	if err != nil {
		...
		FormAndSendHttpResp(resp, "Failed to get teams data from DB", http.StatusBadRequest, nil)
		return
	}
	...
	// Send the response
	log.FuncInfoTrace(0, "Number of Teams list fetched : %v teamlist %+v", len(teamsList.TeamsList), teamsList)
